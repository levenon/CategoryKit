...
- (BOOL)textView:(id<XLFTextInput>)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text;{
    ...
    if (result) {
        ...
        [textView setCorrect:[[self class] contentAllowTextInput:textView content:destination]];
    }
...
- (BOOL)textField:(id<XLFTextInput>)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string;{
    ...
    if (result) {
        ...
        [textField setCorrect:[[self class] contentAllowTextInput:textField content:destination]];
    }
...
+ (BOOL)contentAllowTextInput:(id<XLFTextInput>)textInput{
    ...
    return [self contentAllowTextInput:textInput content:[textInput text]];
}
...
+ (BOOL)contentAllowTextInput:(id<XLFTextInput>)textInput content:(NSString *)content{
    ...
    switch ([textInput textLimitInputType]) {
        ...
        case XLFTextLimitInputTypeEnglish:       // 纯英文
            correct = [content isEnglishCharacter];
            break;
            
        case XLFTextLimitInputTypeChinese:       // 纯中文
            correct = [content isChineseCharacter];
            break;
            
        case XLFTextLimitInputTypeNumber:        // 纯数字
            correct = [content isNumber];
            break;
            
        case XLFTextLimitInputTypeFloatNumber:      // 小数
            correct = [content isNumberOrDecimals];
            break;
            
        case XLFTextLimitInputTypeTelephoneNumber:  // 手机号码
            correct = [content isTelephoneNumber];
            break;
            
        case XLFTextLimitInputTypeEmail:            // 邮箱
            correct = [content isFullEmail];
            break;
            
        case XLFTextLimitInputTypeEnglishOrChinese:// 中英混合
            correct = [content isChineseOrEnglishCharacter];
            break;
            
        case XLFTextLimitInputTypeEnglishOrNumber: // 英文和数字混合
            correct = [content isNumberOrEnglishCharacter];
            break;
        ...
        case XLFTextLimitInputTypeEnglishAndNumber: // 英文和数字混合
            correct = [content isNumberAndEnglishCharacter];
            break;
    }
    
    return correct && [content length] >= [textInput minusSize];
}
